...
DROP SCHEMA IF EXISTS "comercio-oltp";

CREATE SCHEMA "comercio-oltp"  AUTHORIZATION "miguel";

DROP TABLE IF EXISTS  "comercio-oltp".cliente;
...
create table "comercio-oltp".endereco(
	idendereco integer unique not null,
...
alter table "comercio-oltp".endereco add constraint "chave_estrangeira_endereco_cliente" foreign key (id_cliente) references cliente (idcliente) ;


drop table if exists "comercio-oltp".gerente cascade;

create table "comercio-oltp".gerente(
	idgerente integer unique not null,
	nome varchar(30) not null,
...
alter table "comercio-oltp".vendedor add constraint "chave_estrangeira_vendedor_gerente" foreign key (id_gerente) references gerente (idgerente) ;
 

drop table if exists "comercio-oltp".categoria cascade;

create table "comercio-oltp".categoria(
	idcategoria integer unique not null,
...
create table "comercio-oltp".fornecedor(
	idfornecedor integer unique not null,
	nome varchar(200)
);
...
alter table "comercio-oltp".produto add column id_categoria  integer unique;
alter table "comercio-oltp".produto add column id_fornecedor integer unique;
alter table "comercio-oltp".produto add constraint "chave_primaria_produto" primary key (idproduto);
alter table "comercio-oltp".produto add constraint "chave_estrangeira_produto_categoria"  foreign key (id_categoria)  references categoria  (idcategoria);
alter table "comercio-oltp".produto add constraint "chave_estrangeira_produto_fornecedor" foreign key (id_fornecedor) references fornecedor (idfornecedor);


drop table if exists "comercio-oltp".forma_pagamento cascade;

create table "comercio-oltp".forma_pagamento(
	idforma integer unique not null,
...
create table "comercio-oltp".nota_fiscal(
	idnota integer unique not null,
	data date,
	total numeric(10,2),
	id_forma int,
	id_cliente int,
...
create table "comercio-oltp".item_nota(
	iditemnota int unique not null,
	quantidade int,
	total numeric(10,2)
);
...
alter table "comercio-oltp".item_nota add constraint "chave_estrangeira_item_nota_produto"    foreign key (id_produto)     references produto  (idproduto);
alter table "comercio-oltp".item_nota add constraint "chave_estrangeira_item_nota_nota_fiscal" foreign key (id_nota_fiscal) references nota_fiscal (idnota);
